...
try {
    ...
    db = new Sqlite3.Database(path.join(app_path, `../database.db3`))
} catch (err) {
    const { createFallbackDB, createFallbackStore } = require("./json_db_fallback.cjs");
    db = createFallbackDB(path.join(app_path, `../database.json`));
    SQLiteStore = (sess) => createFallbackStore(sess, db);
}
...
function launchServer() {
    return new Promise(async (resolve, reject) => {
        let attemptedPort = settings.server_port;
        const startOnPort = (port) => {
            server.once("error", async (err) => {
                if (err && err.code === "EADDRINUSE") {
                    const fallbackPort = port + 1;
                    settings.server_port = fallbackPort;
                    try {
                        await dbRunWithValues('UPDATE options SET data = ? WHERE id = 1', JSON.stringify(settings));
                    } catch (_) {}
